function env = mAttitudeControlEnv()
% Creates the Attitude Control Environment using rlFunctionEnv
    ...
    obsInfo.Description = '[q1; q2; q3; q4; wx; wy; wz]'; % Quaternion then omega
    ...
    % Group parameters for clarity and access within callbacks
    params.I = [0.872; 0.115; 0.797]; % Ensure COLUMN vector for diag()
    params.h = 1/240; % Simulation step time
    % Note: Agent's action is assumed to be [-1, 1] and will be scaled
    % If agent directly outputs torque, set torqueScale = 1
    params.torqueScale = 0.5; % Scales agent action [-1,1] to torque
    params.frameskipNum = 19; % Dynamics steps = 1 (action) + 19 (free rotate) = 20 per agent step
    params.maxAngleSlew = 150;  % degrees
    params.minAngleSlew = 30;   % degrees
    params.alpha = [-1; -1; -1; 1]; % Reward weights [q1^2, q2^2, q3^2, q4^2]
    params.maxEpisodeSteps = 500;   % Max agent steps before truncation
    params.maxAngVelo = 0.5;        % rad/s failure limit
    params.rewardThresholdQs = cos(deg2rad(0.25)/2); % Threshold for switching reward (qs corresponding to 0.25 deg)


    % --- Create Environment using Function Handles ---
    % Pass parameters struct to the step/reset functions via the environment handle
    env = rlFunctionEnv(obsInfo, actInfo, @myStepFunction, @myResetFunction, params);

end

% =========================================================================
% REQUIRED CALLBACK FUNCTIONS (defined locally or as separate files)
% =========================================================================

function [NextObservation, Reward, IsDone, LoggedSignals] = myStepFunction(Action, LoggedSignals, params)
% Implements the step logic for the environment
    ...
    nSteps = LoggedSignals.nSteps;
    qsPrev = LoggedSignals.qsPrev;
    stepsBeyondDone = LoggedSignals.stepsBeyondDone;
    isInStabilizationPhase = LoggedSignals.isInStabilizationPhase;

    % --- Update Step Counter ---
    nSteps = nSteps + 1;

    % --- Process Action ---
    % Agent outputs action in range [-1, 1] (based on typical actor output)
    % Scale action to torque and clamp to physical limits
    scaled_action = params.torqueScale * Action;
    torque = max(min(scaled_action, params.MaxTorque), -params.MaxTorque); % Apply physical limits
    ...
    NextObservation = [quat; omega]; % Matches obsInfo spec [q1;q2;q3;q4; wx;wy;wz]

    % --- Calculate Reward ---
    qs_next = quat(4); % Scalar part
    omegaMag = norm(omega);

    % Update stabilization phase flag (based on SCALAR part)
    if abs(qs_next) >= params.rewardThresholdQs % Use absolute value as qs can be negative
         isInStabilizationPhase = true;
    % else % Optional: allow falling out of stabilization phase if error grows again
    %     isInStabilizationPhase = false;
    end

    % Determine reward based on phase
    if ~isInStabilizationPhase
        % Phase 1 Reward (Eq 14 logic)
        % Reward getting closer to target (qs magnitude increasing towards 1)
        ...
        % Phase 2 Reward (Eq 15 logic) - Reward being close to target
        % Ensure element-wise power
        Reward = params.alpha' * (quat.^2); % -q1^2 -q2^2 -q3^2 + q4^2
    end

    % --- Check Termination/Truncation ---
    IsDoneVelocity = omegaMag > params.maxAngVelo;
    IsDoneSteps = nSteps >= params.maxEpisodeSteps;
    IsDone = IsDoneVelocity || IsDoneSteps; % Episode ends if limit exceeded or steps maxed out

    % --- Apply Terminal Penalties/Rewards ---
    if IsDone
        if isempty(stepsBeyondDone) % First time IsDone is true
            stepsBeyondDone = 0;
            if IsDoneVelocity
                Reward = -25; % Velocity limit exceeded penalty
            elseif isInStabilizationPhase % Finished at max steps while stable
                Reward = 10; % Success bonus
            % else % Finished at max steps but not stable (optional penalty?)
                 % Reward = -5; % Example
            end
        else
            % If already done, subsequent steps shouldn't happen in standard loop,
            % but if they did, reward is 0.
            stepsBeyondDone = stepsBeyondDone + 1;
            Reward = 0;
        end
    else
         stepsBeyondDone = []; % Reset if not done
    end
...
% =========================================================================
% HELPER FUNCTIONS (defined locally or as separate files)
% =========================================================================
...
function qDot = local_getQDot(omega, quat)
    ...
    % Kinematic Equation: q_dot = 0.5 * Omega_Matrix(omega) * q
    % Or: q_dot = 0.5 * quatmultiply([omega; 0], quat) <- Often clearer
...
function [omega1, q1] = local_RK4_omega_quat(h, omega0, q0, torque, I_col_vec)
    ...
    % Note: Intermediate quaternion normalization is important for stability
    ...
    % K2 calculation
    omega_k2 = omega0 + 0.5 * h * k1_omega_dot;
    q_k2_un = q0 + 0.5 * h * k1_q_dot;
    q_k2 = local_normalizeQuat(q_k2_un); % Normalize intermediate quaternion
    k2_omega_dot = local_getOmegaDot(omega_k2, torque, I_col_vec);
    k2_q_dot = local_getQDot(omega_k2, q_k2);

    % K3 calculation
    omega_k3 = omega0 + 0.5 * h * k2_omega_dot;
    q_k3_un = q0 + 0.5 * h * k2_q_dot;
    q_k3 = local_normalizeQuat(q_k3_un); % Normalize intermediate quaternion
    k3_omega_dot = local_getOmegaDot(omega_k3, torque, I_col_vec);
    k3_q_dot = local_getQDot(omega_k3, q_k3);

    % K4 calculation
    omega_k4 = omega0 + h * k3_omega_dot;
    q_k4_un = q0 + h * k3_q_dot;
    q_k4 = local_normalizeQuat(q_k4_un); % Normalize intermediate quaternion
    k4_omega_dot = local_getOmegaDot(omega_k4, torque, I_col_vec);
    k4_q_dot = local_getQDot(omega_k4, q_k4);
